...
public class Administrator {
    ...
    public int checkPassword() {
        ...
        while (chance != 0) {
            ...
            if (!answer.equals(this.getPassword())) {
                System.out.println("비밀번호가 일치하지 않습니다. 다시 입력해 주세요.\n3회 이상 실패할 경우 메인으로 돌아갑니다.");
                chance--;
                System.out.println("남은 횟수: " + chance);
            } else if (answer.equals(this.getPassword())) {
                break;
            }
        }
        return chance;
    }
    public Cart adminMode(Cart cart, List<Category> categories) {
//        Administrator admin = new Administrator();
        chance = checkPassword();
        if (chance == 0) {
    ...
    public Product getNewProduct(List<Category> categories) {
        ...
        choice = input.nextInt();
        if(choice == 1) {
            categories.get(index).getCategoryList().add(product);
        }
        else if(choice == 2) {
